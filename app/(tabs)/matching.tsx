...
export default function MatchingScreen() {
  ...
  const initGame = useCallback(() => {
    ...
    // --- УМНЫЙ АЛГОРИТМ ВЫБОРА СЛОВ ---
    
    // 1. Находим слова, которые пора повторить (DUE)
    const dueWords = words.filter(w => w.nextReview <= Date.now());
    
    // 2. Находим новые слова (Level 0)
    const newWords = words.filter(w => w.level === 0 && !dueWords.some(d => d.id === w.id));

    // Начинаем собирать выборку из 5 слов
    let selection = [...dueWords].sort(() => 0.5 - Math.random()).slice(0, 5);

    // Если DUE слов меньше 5, докидываем из NEW
    ...
    // Если всё еще меньше 5 (редкий случай), докидываем рандом из всей базы
    if (selection.length < 5) {
      const remaining = words.filter(w => !selection.some(s => s.id === w.id));
    ...
    // --- ПОДГОТОВКА ЭЛЕМЕНТОВ ИНТЕРФЕЙСА ---

    const left = selection.map(w => ({
      id: `word-${w.id}`,
      text: w.word,
      type: 'word' as const,
      originalId: w.id
    ...
    <View style={[styles.container, { backgroundColor: currentTheme.background }]}>
      ...
      <View style={styles.gameContainer}>
        <View style={styles.column}>
          {leftItems.map((item, index) => {
            ...
            if (isSolved) return <View key={item.id} style={styles.placeholder} />;
              ...
              >
                <TouchableOpacity
                  style={[
                    styles.item,
                    { 
                      backgroundColor: isSelected ? currentTheme.primary : currentTheme.card,
                      borderColor: isSelected ? currentTheme.primary : currentTheme.border
                    }
                  ]}
                  onPress={() => handleSelect(item)}
                >
                  <Text style={[styles.itemText, { color: isSelected ? '#fff' : currentTheme.text }]}>
                    {item.text}
                  </Text>
                </TouchableOpacity>
              </Animated.View>
      ...
      {solvedIds.length === 5 && (
        <Animated.View entering={FadeInDown} style={styles.winContainer}>
          <Zap size={40} color={currentTheme.primary} fill={currentTheme.primary} />
          <Text style={[styles.winText, { color: currentTheme.text }]}>Все верно!</Text>
          <TouchableOpacity 
...
const styles = StyleSheet.create({
  ...
  gameContainer: { flexDirection: 'row', justifyContent: 'space-between', flex: 1 },
  ...
  emptyText: { textAlign: 'center', marginTop: 100, fontSize: 18, fontWeight: '600' },
  winContainer: { 
    position: 'absolute', 
    bottom: 50, 
    left: 20, 
