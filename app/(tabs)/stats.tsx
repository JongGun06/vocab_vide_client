...
export default function StatsScreen() {
  ...
  const stats = useMemo(() => {
    ...
    const accuracy =
      allReviews > 0 ? Math.round((successfulReviews / allReviews) * 100) : 0;
  ...
  const StatCard = ({ title, value, icon: Icon, color, delay }: any) => (
    ...
    >
      ...
      <Text style={[styles.statTitle, { color: currentTheme.textSecondary }]}>
        {title}
      </Text>
    </Animated.View>
  );
  ...
  return (
    ...
    >
      ...
      <Animated.View
        ...
        style={[
          styles.streakCard,
          { backgroundColor: currentTheme.card, borderColor: "#FF4500" },
        ]}
      >
        <Flame size={32} color="#FF4500" fill="#FF4500" />
        <View>
          <Text style={[styles.streakValue, { color: currentTheme.text }]}>
            {streak} дней
          </Text>
          <Text
            style={[styles.streakTitle, { color: currentTheme.textSecondary }]}
          >
            Ударная серия
          </Text>
        </View>
      ...
      <Animated.View entering={FadeInDown.delay(450)} style={styles.section}>
        ...
        >
          ...
          <Text
            style={[styles.heatHint, { color: currentTheme.textSecondary }]}
          >
      ...
      <Animated.View entering={FadeInDown.delay(500)} style={styles.section}>
        ...
        >
          {[
            ...
            {
              label: "Уровень 1",
              count: words.filter((w) => w.level === 1).length,
              color: "#3B82F6",
            },
            {
              label: "Уровень 2",
              count: words.filter((w) => w.level === 2).length,
              color: "#A855F7",
            },
            ...
            <View key={i} style={styles.levelRow}>
              <View style={styles.levelLabelBox}>
                ...
                <Text style={[styles.levelLabel, { color: currentTheme.text }]}>
                  {lvl.label}
                </Text>
              </View>
